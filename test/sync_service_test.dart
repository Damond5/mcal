...
void main() {
  ...
  setUp(() async {
    ...
    // Restore custom flutter_secure_storage mock for test needs
    ...
    when(
      mockApi.crateApiGitInit(path: anyNamed('path')),
    ).thenAnswer((_) async => 'Initialized');
    when(
      mockApi.crateApiGitAddRemote(
        path: anyNamed('path'),
        name: anyNamed('name'),
        url: anyNamed('url'),
      ),
    ).thenAnswer((_) async => 'Remote added');
    when(
      mockApi.crateApiGitFetch(
        path: anyNamed('path'),
        remote: anyNamed('remote'),
        username: anyNamed('username'),
        password: anyNamed('password'),
        sshKeyPath: anyNamed('sshKeyPath'),
      ),
    ).thenAnswer((_) async => 'Fetched');
    when(
      mockApi.crateApiGitCheckout(
        path: anyNamed('path'),
        branch: anyNamed('branch'),
      ),
    ).thenAnswer((_) async => 'Checked out');
  });
  ...
  test('initSync stores remote URL', () async {
    ...
    await expectLater(
      syncService.initSync('https://example.com/repo.git'),
      completes,
    );
  });
  ...
  test('pullSync calls gitPull with auth', () async {
    ...
    when(
      mockApi.crateApiGitPull(
        path: anyNamed('path'),
        username: anyNamed('username'),
        password: anyNamed('password'),
        sshKeyPath: anyNamed('sshKeyPath'),
      ),
    ).thenAnswer((_) async => 'Pulled');
  ...
  test('pushSync calls gitPush with auth', () async {
    ...
    when(mockApi.crateApiGitStatus(path: anyNamed('path'))).thenAnswer(
      (_) async => [StatusEntry(path: 'file.txt', status: 'modified')],
    );
    when(
      mockApi.crateApiGitAddAll(path: anyNamed('path')),
    ).thenAnswer((_) async => 'Added');
    when(
      mockApi.crateApiGitCommit(
        path: anyNamed('path'),
        message: anyNamed('message'),
      ),
    ).thenAnswer((_) async => 'Committed');
    when(
      mockApi.crateApiGitPush(
        path: anyNamed('path'),
        username: anyNamed('username'),
        password: anyNamed('password'),
        sshKeyPath: anyNamed('sshKeyPath'),
  ...
  test('getSyncStatus returns string', () async {
    when(
      mockApi.crateApiGitStatus(path: anyNamed('path')),
    ).thenAnswer((_) async => []);
  ...
  test('resolveConflictPreferRemote calls gitMergePreferRemote', () async {
    when(
      mockApi.crateApiGitMergePreferRemote(path: anyNamed('path')),
  ...
  test('abortConflict calls gitMergeAbort', () async {
    when(
      mockApi.crateApiGitMergeAbort(path: anyNamed('path')),
